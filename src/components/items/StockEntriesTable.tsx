...
export function StockEntriesTable({ entries, items, suppliers, onView }: StockEntriesTableProps) {
  if (entries.length === 0) {
    return (
      ...
      <table className="w-full text-sm" role="table" aria-label="Stock entries list">
        <thead>
          <tr className="border-b bg-muted/30">
            ...
            <th
              scope="col"
              className="w-12 px-3 py-3 text-center font-medium text-muted-foreground"
            >
              Actions
            </th>
          </tr>
        ...
        <tbody>
          {entries.map((entry) => (
            ...
            >
              <td className="px-4 py-3 font-medium sm:px-6">{getItemName(entry, items)}</td>
              <td className="px-3 py-3 text-muted-foreground">{formatDate(entry.purchaseDate)}</td>
