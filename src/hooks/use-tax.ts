...
export function useGSTSummary(startDate: string, endDate: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<GSTSummaryData>(`/tax/gst-summary?startDate=${startDate}&endDate=${endDate}`);
      return res.data;
    },
...
export function useGSTItemized(startDate: string, endDate: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<GSTItemizedData>(`/tax/itemized-report?startDate=${startDate}&endDate=${endDate}`);
      return res.data;
    },
    enabled: !!startDate && !!endDate,
  });
